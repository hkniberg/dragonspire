...
export class EventCardResolver {
   ...
   */
  private async resolveHungryPests(gameState: GameState, currentPlayer: Player): Promise<EventResolutionResult> {
    ...
    if (!chosenPlayer || !otherPlayers.find((p) => p.id === chosenPlayer.id)) {
      ...
      const updatedPlayers = gameState.players.map((p) => (p.id === fallbackPlayer.id ? updatedFallbackPlayer : p));
    ...
    const updatedPlayers = gameState.players.map((p) => (p.id === chosenPlayer.id ? updatedChosenPlayer : p));
