...
abstract class GameActionTool implements Tool {
  ...
  constructor(
    protected executeAction: ExecuteActionFunction,
...
export class MoveChampionTool extends GameActionTool {
  name = "moveChampion";
  description =
    "Move a champion from the current position to a given end position, through the given path (including start position). Optionally claim the destination tile if it's an unclaimed resource tile.";
  inputSchema = {
    type: "object",
    properties: {
      championId: {
      ...
      path: {
        type: "array",
        description:
          "Array of positions the champion will move through, from start position (inclusive) to end position (inclusive)",
        items: {
          type: "object",
          properties: {
            row: { type: "number" },
