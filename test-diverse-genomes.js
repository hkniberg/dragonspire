...
async function testArchetype(name, genome, games = 3) {
  ...
  console.log(
    `📊 ${name} Results: ${winRate * 100}% wins, ${avgPlacement.toFixed(
      1
    )} avg placement`
  );
...
async function main() {
  console.log(
    "🔬 Testing diverse genome archetypes to explore strategy space...\n"
  );
  ...
  sorted.forEach(([name, result], index) => {
    ...
    console.log(
      `${index + 1}. ${name.padEnd(20)} | ${(result.winRate * 100).toFixed(
        0
      )}% wins | ${result.avgPlacement.toFixed(1)} avg | Score: ${score.toFixed(
        1
      )}`
    );
  });
